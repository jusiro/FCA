...
def process(args):
    ...
    for i_task in args.tasks:
        ...
        for _ in tqdm(range(args.seeds), leave=False, desc="  Conformal inference: "):
            ...
            logits_test, labels_test = cache_test["logits_ds"], np.int8(cache_test["refs_ds"])
            ...
            logits_calib, labels_calib = conformal.balance_split(
                cache_adapt["logits_ds"], np.int8(cache_adapt["refs_ds"]), k=args.k, p=label_dist, seed=_)

            # Move data to gpu
            logits_test, labels_test = torch.tensor(logits_test), torch.tensor(labels_test).to(torch.long)
            logits_calib, labels_calib = torch.tensor(logits_calib), torch.tensor(labels_calib).to(torch.long)
            ...
            # No adaptation - softmax of logit scores
            z = torch.softmax(logits_ds, dim=-1)
    ...
    path = "./local_data/results/scp/{alpha}/{ncscore}/summary/".format(
    ...
    # save detailed results
    path = "./local_data/results/scp/{alpha}/{ncscore}/detailed/".format(
        alpha=str(args.alpha).replace(".", ""), ncscore=args.ncscore)
    if not os.path.exists(path):
        os.makedirs(path)
