...
def process(args):
    ...
    for i_task in args.tasks:
        ...
        for _ in tqdm(range(args.seeds), leave=False, desc="  Conformal inference: "):
            ...
            # Full conformal prediction inference
            time_adapt_i_1 = time.time()

            # Run over samples and labels
            val_sets, preds_test = [], []
            preds_test_avg = []
            for i_sample in tqdm(range(feats_test.size(0))):
                i_sample_set, i_sample_pred, label_confidence = [], [], []
                for i_label in list(np.unique(labels_test)):

                    # Set training set
                    feats_joint = torch.concat([feats_calib,  feats_test[i_sample, :].unsqueeze(0)], dim=0)
                    labels_joint = torch.concat([labels_calib, torch.tensor(i_label).unsqueeze(0)], dim=0)

                    # Set Adapter
                    adapter = Adapter(torch.tensor(cache_adapt["initial_prototypes"]),
                                      cache_adapt["logit_scale"], adapter=args.adapt)

                    # Adjust Adapter and produce predictions
                    z, _ = sstext.adapt(feats_joint, labels_joint, adapter)

                    # Retrieve calibration and validation predictions
                    preds_calib_i, preds_test_i = z[:len(labels_calib), :], z[len(labels_calib):, :]

                    # apply the conformal algorithms
                    test_sets, time_fit_i, time_infer_i = conformal.conformal_method(
                        args.ncscore, preds_calib_i, labels_calib, preds_test_i, args.alpha)

                    # Add prediction
                    i_sample_pred.append(preds_test_i.unsqueeze(1))

                    # Check if target label is on the set, and include it
                    if i_label in test_sets[0]:
                        i_sample_set.append(i_label)

                # Compute prediction
                preds_test_avg.append(torch.concat(i_sample_pred, 0).mean(0))  # Average code acros label assignment
                # Add conformal set
                val_sets.append(i_sample_set)

            # Set transfer learning times
            time_adapt_i_2 = time.time()
            time_adapt_i = time_adapt_i_2 - time_adapt_i_1
            time_adapt_i /= feats_test.size(0)  # Normalization per image

            # Concatenate individual predictions
            preds_test = torch.concat(preds_test_avg, 0)
    ...
    path = "./local_data/results/fca/{alpha}/{ncscore}/summary/".format(
    ...
    # save detailed results
    path = "./local_data/results/fca/{alpha}/{ncscore}/detailed/".format(
        alpha=str(args.alpha).replace(".", ""), ncscore=args.ncscore)
    if not os.path.exists(path):
        os.makedirs(path)
